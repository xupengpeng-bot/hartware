...
static uint8_t workflow_card_reader_is_visible_ascii(uint8_t ch)
{
    return (uint8_t)(((ch >= 0x20U && ch <= 0x7EU) || ch == '\t') ? 1U : 0U);
}

static size_t workflow_card_reader_find_binary_resync_offset(void)
{
    size_t  idx;
    uint8_t saw_non_ascii = 0U;

    for (idx = 0U; idx < s_rx_len; ++idx) {
        uint8_t ch = s_rx_buffer[idx];
        ...
        if (ch == 0x20U) {
            ...
            if ((s_rx_len - idx) >= 13U && workflow_card_reader_checksum_ok(&s_rx_buffer[idx], 13U) != 0U) {
                return idx;
            }
        }
        if (workflow_card_reader_is_visible_ascii(ch) == 0U) {
            saw_non_ascii = 1U;
        }
    }
...
static size_t workflow_card_reader_try_parse_ascii(uint32_t now_ms)
{
    ...
    if (idx >= s_rx_len) {
        resync_offset = workflow_card_reader_find_binary_resync_offset();
        if (resync_offset != (size_t)(~0U) && resync_offset > 0U) {
            workflow_card_reader_logf("resync drop=%lu total=%lu\r\n",
                                      "",
                                      (unsigned long)resync_offset,
                                      (unsigned long)s_rx_len);
            return resync_offset;
        }
    ...
    while (s_rx_len > 0U) {
        ...
        if (resync_offset != (size_t)(~0U) && resync_offset > 0U) {
            workflow_card_reader_logf("resync drop=%lu total=%lu\r\n",
                                      ...
                                      (unsigned long)resync_offset,
                                      (unsigned long)s_rx_len);
            workflow_card_reader_consume(resync_offset);
            continue;
        }
