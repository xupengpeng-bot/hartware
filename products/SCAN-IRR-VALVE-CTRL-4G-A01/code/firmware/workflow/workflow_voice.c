...
#define WORKFLOW_VOICE_RETRY_BACKOFF_MS 200U
...
static bool workflow_voice_local_hold_active(void)
{
    return s_busy_until_ms != 0U && (int32_t)(s_busy_until_ms - s_now_ms) > 0;
}
...
static void workflow_voice_queue_push_front(const workflow_voice_queue_item_t *item)
{
    if (!item || s_queue_len >= WORKFLOW_VOICE_QUEUE_CAP) {
        return;
    }

    s_queue_head = (uint8_t)((s_queue_head + WORKFLOW_VOICE_QUEUE_CAP - 1U) % WORKFLOW_VOICE_QUEUE_CAP);
    s_queue[s_queue_head] = *item;
    s_queue_len++;
    s_state.queue_depth = s_queue_len;
}
...
void workflow_voice_tick(uint32_t now_ms)
{
    ...
    s_now_ms = now_ms;
    s_state.supported = bsp_voice_supported();
    s_state.busy = workflow_voice_local_hold_active();
    s_state.queue_depth = s_queue_len;

    if (workflow_voice_local_hold_active()) {
        return;
    }
    ...
    if (rc < 0) {
        workflow_voice_queue_push_front(&item);
        s_busy_until_ms = now_ms + WORKFLOW_VOICE_RETRY_BACKOFF_MS;
        s_state.busy = true;
        return;
    }
...
{
    return workflow_voice_local_hold_active();
}
