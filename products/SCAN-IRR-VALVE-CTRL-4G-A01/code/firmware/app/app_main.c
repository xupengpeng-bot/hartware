...
static void cb_command_nack(const char *json, size_t len, void *user)
{
    char reject_code[24];
    char reject_msg[64];
    ...
    reject_code[0] = '\0';
    reject_msg[0] = '\0';
    (void)proto_json_get_string(json, "rc", reject_code, sizeof(reject_code));
    (void)proto_json_get_string(json, "msg", reject_msg, sizeof(reject_msg));
    if (reject_code[0] != '\0' || reject_msg[0] != '\0') {
        char line[176];

        (void)snprintf(line, sizeof(line),
                       "[PROTO] downlink COMMAND_NACK received rc=%s msg=%s\r\n",
                       reject_code[0] != '\0' ? reject_code : "-",
                       reject_msg[0] != '\0' ? reject_msg : "-");
        bsp_debug_log(line);
        return;
    }
    bsp_debug_log("[PROTO] downlink COMMAND_NACK received\r\n");
}
...
static const proto_dispatch_handlers_t s_proto_handlers = {
    ...
    .on_command_nack         = cb_command_nack,
