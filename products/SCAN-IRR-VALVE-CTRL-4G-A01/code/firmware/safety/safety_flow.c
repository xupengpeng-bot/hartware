...
static bool safety_card_auth_waiting_platform_result(void);
static bool safety_card_action_is_start_followup(const char *action_code);
static bool safety_card_action_is_stop_followup(const char *action_code);
static bool safety_card_prompt_is_explicit_reject(const char *prompt_code);
static void safety_complete_pending_card_auth_from_platform(const char *action_code,
                                                            const char *session_ref,
                                                            bool speak_auth_granted);
static void safety_reject_pending_card_auth_from_platform(const char *reason_code);
#if BOARD_HW_HAS_ELECTRIC_METER
static bool safety_meter_feature_enabled(void);
#else
#define safety_meter_feature_enabled() false
#endif
...
static bool safety_card_auth_waiting_platform_result(void)
{
    return s_card_ctx.state == CARD_STATE_CLOUD_AUTH_PENDING;
}

static bool safety_card_action_is_start_followup(const char *action_code)
{
    return action_code != NULL &&
           (strcmp(action_code, "start_session") == 0 ||
            strcmp(action_code, "start_pump") == 0 ||
            strcmp(action_code, "open_valve") == 0);
}

static bool safety_card_action_is_stop_followup(const char *action_code)
{
    return action_code != NULL &&
           (strcmp(action_code, "stop_session") == 0 ||
            strcmp(action_code, "stop_pump") == 0 ||
            strcmp(action_code, "close_valve") == 0);
}

static bool safety_card_prompt_is_explicit_reject(const char *prompt_code)
{
    return prompt_code != NULL &&
           (strcmp(prompt_code, "invalid_card") == 0 ||
            strcmp(prompt_code, "insufficient_balance") == 0 ||
            strcmp(prompt_code, "port_busy") == 0 ||
            strcmp(prompt_code, "device_fault") == 0 ||
            strcmp(prompt_code, "unavailable") == 0 ||
            strcmp(prompt_code, "network_unavailable") == 0 ||
            strcmp(prompt_code, "auth_denied") == 0);
}
                                                            ...
                                                            bool speak_auth_granted)
{
    ...
    if (!safety_card_auth_waiting_platform_result()) {
        return;
    }

    if (session_ref != NULL && session_ref[0] != '\0') {
        safety_copy_string(s_card_ctx.pending_session_ref, sizeof(s_card_ctx.pending_session_ref), session_ref);
    }

    newly_granted = (s_card_ctx.auth_granted == 0U);
    s_card_ctx.auth_granted = 1U;
    ...
    if (speak_auth_granted && newly_granted) {
        safety_voice_prompt(VOICE_AUTH_GRANTED);
    }
    safety_logf("[FLOW] ", "card swipe accepted via platform action=%s token=%s session=%s",
...
static void safety_reject_pending_card_auth_from_platform(const char *reason_code)
{
    ...
    s_card_ctx.auth_granted = 0U;
    runtime_state_set_last_event(EVT_CLOUD_AUTH_DENIED);
    safety_logf("[FLOW] ", "card swipe denied via platform feedback=%s token=%s session=%s",
                reason_code != NULL ? reason_code : "platform_denied",
                s_card_ctx.card_no,
                s_card_ctx.pending_session_ref);
    safety_queue_card_auth_report("platform_denied",
                                  (reason_code != NULL && reason_code[0] != '\0') ? reason_code : "platform_denied");
...
static void safety_bootstrap_meter_epoch(void)
{
    ...
    runtime_state_set_meter_epoch(0U);
}
...
static bool safety_meter_feature_enabled(void)
{
    ...
    return (cfg != NULL && cfg->feature_modules.electric_meter_modbus != 0U);
}
#endif

static const char *safety_meter_source_name(void)
{
    return safety_meter_feature_enabled() ? "meter" : "disabled";
}
...
static const char *safety_valve_module_code(void)
{
    const device_config_t *cfg = config_store_active();
...
{
    return SETTLEMENT_BY_DURATION_HOUR;
}
...
static void safety_track_meter_epoch_events(void)
{
    s_flow_ctx.meter_energy_seen = 0U;
    s_flow_ctx.last_seen_energy_kwh = 0.0f;
}
...
static int safety_execute_start_sequence(session_target_type_t target, linkage_mode_t linkage)
{
    ...
    const char *valve_module_code = safety_valve_module_code();
    ...
    if (safety_target_has_valve(target)) {
        ...
        if (valve_module_code == NULL ||
            safety_run_local_action(valve_module_code, "open_valve") != 0U) {
...
static int safety_close_local_outputs(session_target_type_t target, linkage_mode_t linkage)
{
    ...
    if (safety_target_has_valve(target) &&
        ...
        if (valve_module_code == NULL ||
            safety_run_local_action(valve_module_code, "close_valve") != 0U) {
            return SAFETY_FLOW_ERR_SEQUENCE;
        }
        runtime_state_set_valve_state(RUNTIME_VALVE_CLOSED);
    }
...
void safety_flow_on_query_result(const char *json)
{
    ...
    if (proto_json_get_string(json, "result", result, sizeof(result)) != 0 &&
        ...
        if (proto_json_get_string(json, "action", action, sizeof(action)) == 0 && action[0] != '\0') {
            (void)snprintf(result, sizeof(result), "accepted");
        } else {
            (void)proto_json_get_string(json, "error_code", error_code, sizeof(error_code));
            (void)proto_json_get_string(json, "prompt_code", prompt_code, sizeof(prompt_code));
            if (error_code[0] == '\0' && !safety_card_prompt_is_explicit_reject(prompt_code)) {
                return;
            }
            (void)snprintf(result, sizeof(result), "rejected");
        }
    }
    reject_reason[0] = '\0';
    ...
    if (reject_reason[0] == '\0' && error_code[0] != '\0') {
        safety_copy_symbol(reject_reason, sizeof(reject_reason), error_code);
    }
    if (reject_reason[0] == '\0' && prompt_code[0] != '\0') {
        safety_copy_symbol(reject_reason, sizeof(reject_reason), prompt_code);
    }

    if (strcmp(result, "allowed") == 0 || strcmp(result, "accepted") == 0 ||
        strcmp(result, "granted") == 0 || strcmp(result, "ok") == 0 ||
        strcmp(result, "success") == 0) {
        safety_complete_pending_card_auth_from_platform(action[0] != '\0' ? action : "query_result", NULL, true);
    } else {
        safety_voice_prompt(VOICE_AUTH_DENIED);
        safety_reject_pending_card_auth_from_platform(reject_reason[0] != '\0' ? reject_reason : "platform_denied");
    }
}
                               ...
                               char *detail_json, size_t detail_cap)
{
    ...
    if (strcmp(action_code, "voice_broadcast") == 0) {
        ...
        if (json != NULL && proto_json_get_string(json, "prompt_code", prompt_code, sizeof(prompt_code)) == 0) {
            if (safety_card_prompt_is_explicit_reject(prompt_code)) {
                safety_reject_pending_card_auth_from_platform(prompt_code);
            } else if (strcmp(prompt_code, "auth_granted") == 0) {
                safety_complete_pending_card_auth_from_platform("voice_broadcast", session_ref, false);
            }
    ...
    if (strcmp(action_code, "stop_session") == 0) {
        if (safety_card_action_is_stop_followup(action_code)) {
            safety_complete_pending_card_auth_from_platform(action_code, session_ref, false);
        }
    ...
    if (safety_card_action_is_start_followup(action_code)) {
        safety_complete_pending_card_auth_from_platform(action_code, session_ref, true);
    }
