...
{
    return scope != NULL && strcmp(scope, "cm") == 0;
}
...
int proto_query_handle(const char *json, size_t json_len, char *reply, size_t reply_cap)
{
    ...
    read_optional_string(payload, "qc", qcode, sizeof(qcode));
    ...
    if (scope_is_common(scope) && strcmp(qcode, "qcs") == 0) {
    ...
    if (scope_is_common(scope) && strcmp(qcode, "qgs") == 0) {
    ...
    if (scope_is_common(scope) && strcmp(qcode, "qgc") == 0) {
