...
static void filter_feature_modules(feature_modules_t *dst, const feature_modules_t *src)
{
    if (dst == NULL) {
        return;
    }
...
static int heartbeat_build_common(char *buf, size_t cap, uint32_t seq)
{
    ...
    feature_modules_t capability_modules;
    ...
    filter_feature_modules(&capability_modules, &cfg->feature_modules);
    if (proto_capability_describe(cfg, &capability_modules, &capability) != 0) {
        return -1;
    }
    ...
    if (meter_enabled != 0U && metric_in_range(rs->energy_kwh, 0.0f, 10000000.0f) != 0U) {
        if (append_optional_separator(&jb, &first) != 0 ||
