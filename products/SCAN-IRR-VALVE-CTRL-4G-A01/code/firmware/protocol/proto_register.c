...
static const capability_desc_t s_capability_defs[] = {
    ...
    { "dual_valve_control",    offsetof(feature_modules_t, dual_valve_control) },
    { "relay_output_control",  offsetof(feature_modules_t, relay_output_control) },
...
static void filter_real_feature_modules(feature_modules_t *dst, const feature_modules_t *src)
{
    ...
    *dst = *src;
    dst->electric_meter_modbus = 0U;
    dst->flow_acquisition = 0U;
    dst->single_valve_control = 0U;
}

static const char *register_control_protocol(const device_config_t *cfg, const feature_modules_t *fm)
{
    if (cfg == NULL || fm == NULL ||
        (fm->pump_vfd_control == 0U && fm->pump_direct_control == 0U && fm->breaker_control == 0U)) {
        return NULL;
    }
...
static int append_resource_inventory_field(json_buf_t *jb, uint8_t *first)
{
    const resource_inventory_t *ri = common_resource_inventory_get();
    uint8_t emitted = 0U;

    if (jb == NULL || first == NULL || ri == NULL) {
        return -1;
    }
    if (ri->relay_output == 0U &&
        ri->motor_driver == 0U &&
        ri->digital_input == 0U &&
        ri->analog_input == 0U &&
        ri->pulse_input == 0U &&
    ...
    *first = 0U;
    if (json_buf_append(jb, "\"ri\":{") != 0) {
        return -1;
    }
#define APPEND_RI_FIELD(name, value) \
    do { \
        if ((value) != 0U) { \
            if (emitted != 0U && json_buf_append(jb, ",") != 0) { \
                return -1; \
            ...
            emitted = 1U; \
            if (json_buf_append_fmt(jb, "\"%s\":%u", (name), (unsigned)(value)) != 0) { \
                return -1; \
            } \
        } \
    } while (0)
    APPEND_RI_FIELD("relay_output", ri->relay_output);
    APPEND_RI_FIELD("motor_driver", ri->motor_driver);
    APPEND_RI_FIELD("digital_input", ri->digital_input);
    APPEND_RI_FIELD("analog_input", ri->analog_input);
    APPEND_RI_FIELD("pulse_input", ri->pulse_input);
    APPEND_RI_FIELD("rs485_modbus", ri->rs485_modbus);
    APPEND_RI_FIELD("power_monitor", ri->power_monitor);
    APPEND_RI_FIELD("card_reader", ri->card_reader);
#undef APPEND_RI_FIELD
    return json_buf_append(jb, "}");
}
...
int proto_register_build(char *buf, size_t cap)
{
    ...
    if (append_resource_inventory_field(&jb, &first) != 0) {
        return -4;
    }
