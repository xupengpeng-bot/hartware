...
#ifndef FW_MODEM_VERBOSE_LOG
#define FW_MODEM_VERBOSE_LOG 1
#endif

#if !FW_MODEM_VERBOSE_LOG
...
#define MODEM_QIRD_HTTP_DATA_BYTE_TIMEOUT_MS 800U
#define MODEM_QIRD_TAIL_TIMEOUT_MS 200U
#define MODEM_QIRD_FETCH_MAX 768U
#define MODEM_QIRD_HTTP_FETCH_MAX 256U
#define MODEM_QIRD_HTTP_PROBE_INTERVAL_MS 200U
#define MODEM_QIRD_PENDING_RETRY_MS 1000U
#define MODEM_QIRD_PENDING_NO_DATA_LIMIT 8U
#define MODEM_QIRD_PROBE_INTERVAL_MS 5000U
#define MODEM_QISEND_PROMPT_TIMEOUT_MS 5000U
#define MODEM_QISEND_RETRY_DELAY_MS 120U
#define MODEM_QIOPEN_READY_DELAY_MS 120U
...
static uint32_t s_last_poll_monotonic_ms;
static uint32_t s_last_downlink_activity_ms;
...
static uint32_t s_last_qird_probe_ms;
static uint8_t  s_qird_fail_streak;
static uint8_t  s_tcp_seen_downlink;
static unsigned s_qird_pending_len;
static uint8_t  s_qird_pending_no_data_streak;
static uint8_t  s_qird_pending_from_probe;
static uint32_t s_qird_pending_retry_after_ms;
static uint16_t s_tcp_remote_port;
static uint8_t  s_http_body_log_suppressed;

typedef struct {
    int      read_len;
    unsigned data_len;
    unsigned unread_len;
    uint8_t  unread_len_known;
} modem_qird_result_t;
...
static void tcp_fifo_reset(void);
...
static size_t modem_ascii_window(const uint8_t *data, size_t len, char *out, size_t out_cap);
...
static int modem_is_http_download_session(void);
static void modem_prepare_at_rx(void);
static int modem_maybe_handle_urc_line(const char *line);
static int modem_qird_query_available(unsigned *total_len, unsigned *unread_len);
static size_t tcp_fifo_used_bytes(void);
static size_t tcp_fifo_free_bytes(void);
static unsigned modem_qird_pick_request_len(unsigned pending_len);
static unsigned modem_qird_pick_direct_request_len(unsigned pending_len, size_t cap);
static void modem_accumulate_pending_recv(unsigned bytes);
static int modem_parse_qird_header_values(const char *line, int *a, int *b, int *c);
static int modem_qird_fetch_common(unsigned request_len, modem_qird_result_t *result,
                                   uint8_t *direct_buf, size_t direct_cap);
static uint32_t modem_qird_payload_byte_timeout_ms(const uint8_t *direct_buf);
...
static void modem_reset_qird_pending_state(void)
{
    ...
    s_qird_pending_from_probe = 0U;
    s_qird_pending_retry_after_ms = 0U;
}

static void modem_note_downlink_activity(void)
{
    s_last_downlink_activity_ms = s_last_poll_monotonic_ms;
    s_tcp_seen_downlink = 1U;
}

static size_t tcp_fifo_used_bytes(void)
{
    if (s_tcp_tail >= s_tcp_head) {
        return s_tcp_tail - s_tcp_head;
    }
    return TCP_RX_FIFO_CAP - (s_tcp_head - s_tcp_tail);
}

static size_t tcp_fifo_free_bytes(void)
{
    size_t used = tcp_fifo_used_bytes();

    if (used >= (TCP_RX_FIFO_CAP - 1U)) {
        return 0U;
    }
    return (TCP_RX_FIFO_CAP - 1U) - used;
}
...
static unsigned modem_qird_pick_request_len(unsigned pending_len)
{
    size_t free_bytes = tcp_fifo_free_bytes();

    if (pending_len == 0U || free_bytes == 0U) {
        return 0U;
    }
    if (pending_len > MODEM_QIRD_FETCH_MAX) {
        pending_len = MODEM_QIRD_FETCH_MAX;
    }
    if (modem_is_http_download_session() != 0 && pending_len > MODEM_QIRD_HTTP_FETCH_MAX) {
        pending_len = MODEM_QIRD_HTTP_FETCH_MAX;
    }
    if ((size_t)pending_len > free_bytes) {
        pending_len = (unsigned)free_bytes;
    }
    return pending_len;
}
...
static unsigned modem_qird_pick_direct_request_len(unsigned pending_len, size_t cap)
{
    ...
    if ((size_t)pending_len > cap) {
        pending_len = (unsigned)cap;
    }
    return pending_len;
}
...
static void modem_accumulate_pending_recv(unsigned bytes)
{
    const unsigned max_pending = (unsigned)(~0U);

    if (bytes == 0U) {
        return;
    }
    if (s_qird_pending_len > (max_pending - bytes)) {
        s_qird_pending_len = max_pending;
        return;
    }
    s_qird_pending_len += bytes;
}
...
static unsigned modem_qird_next_pending_len(unsigned pending_before, const modem_qird_result_t *result,
                                            uint8_t preserve_pending_hint)
{
    unsigned consumed = 0U;
    unsigned advertised_remaining = 0U;
    unsigned hinted_remaining = 0U;

    if (result == NULL) {
        return 0U;
    }
    if (result->read_len > 0) {
        consumed = (unsigned)result->read_len;
    }
    if (result->data_len > consumed) {
        advertised_remaining = result->data_len - consumed;
    }
    if (pending_before > consumed) {
        hinted_remaining = pending_before - consumed;
    }
    if (result->unread_len > 0U) {
        return advertised_remaining + result->unread_len;
    }
    if (result->unread_len_known != 0U) {
        return advertised_remaining;
    }
    if (preserve_pending_hint != 0U && hinted_remaining > advertised_remaining) {
...
static uint32_t modem_qird_payload_byte_timeout_ms(const uint8_t *direct_buf)
{
    if (direct_buf != NULL && modem_is_http_download_session() != 0) {
        return MODEM_QIRD_HTTP_DATA_BYTE_TIMEOUT_MS;
    }
    return MODEM_QIRD_DATA_BYTE_TIMEOUT_MS;
}

static void modem_http_discard_partial_line_residue(void)
{
    if (modem_is_http_download_session() == 0) {
        return;
    }
    /* QIRD payload fetches should start from a clean AT-line boundary.
     * After we have already consumed complete lines, any leftover bytes in the
     * shared modem stream are not a valid standalone URC yet. Keeping those
     * bytes around can leak a stale trailing payload/control byte into the next
     * HTTP fetch seam and duplicate the previous byte in the staged image. */
...
static unsigned modem_http_refresh_pending_after_fetch(unsigned current_pending,
                                                       unsigned request_len,
                                                       const modem_qird_result_t *result)
{
    unsigned probe_total = 0U;
    unsigned probe_unread = 0U;

    if (modem_is_http_download_session() == 0 || current_pending != 0U ||
        request_len == 0U || result == NULL) {
        return current_pending;
    }
    if (result->read_len < (int)request_len) {
        return current_pending;
    }
    if (modem_qird_query_available(&probe_total, &probe_unread) != 0 || probe_unread == 0U) {
        return current_pending;
    }

    {
        ...
        (void)snprintf(linebuf, sizeof(linebuf),
                       "[4G] QIRD HTTP post-fetch probe total=%u unread=%u after fetch=%d/%u\r\n",
                       probe_total,
                       probe_unread,
                       result->read_len,
                       request_len);
    ...
    s_qird_pending_from_probe = 1U;
    return probe_unread;
}

static int modem_http_refresh_pending_after_empty_fetch(unsigned *out_pending)
{
    unsigned probe_total = 0U;
    unsigned probe_unread = 0U;

    if (out_pending == NULL || modem_is_http_download_session() == 0) {
        return 0;
    }
    if (modem_qird_query_available(&probe_total, &probe_unread) != 0 || probe_unread == 0U) {
        return 0;
    ...
    *out_pending = probe_unread;
    ...
    s_qird_pending_retry_after_ms = 0U;
    {
        char linebuf[160];
        (void)snprintf(linebuf, sizeof(linebuf),
                       "[4G] QIRD HTTP empty-fetch probe total=%u unread=%u, resume fetch\r\n",
                       probe_total,
                       probe_unread);
        bsp_debug_log(linebuf);
    }
...
static int modem_is_http_download_session(void)
{
    return (s_tcp_connected != 0 && s_tcp_remote_port == 80U) ? 1 : 0;
}
...
static int modem_qird_query_available(unsigned *total_len, unsigned *unread_len)
{
    ...
    modem_prepare_at_rx();
    if (modem_write_str("AT+QIRD=0,0\r\n") < 0) {
        return -1;
    }
    ...
    while (waited < 3000U) {
        ...
        if (ch == '\n') {
            line[li] = '\0';
            if (li > 0U) {
                if (modem_maybe_handle_urc_line(line) != 0) {
                    li = 0U;
                    continue;
                }
                if (strncmp(line, "+QIRD:", 6) == 0) {
                    unsigned total_hint = 0U;
                    unsigned readable_now = 0U;
                    unsigned remaining_after_read = 0U;

                    fields = modem_parse_qird_header_values(line, &a, &b, &c);
                    modem_log_qird_header(0U, line, a, b, c, fields);
                    if (fields >= 1) {
                        saw_header = 1;
                        total_hint = (unsigned)((a > 0) ? a : 0);
                        readable_now = (unsigned)((b > 0) ? b : 0);
                        remaining_after_read = (unsigned)((c > 0) ? c : 0);
                        if (total_len != NULL) {
                            *total_len = total_hint;
                        }
                        if (unread_len != NULL) {
                            if (fields >= 3) {
                                /* Official Quectel semantics for AT+QIRD=<id>,0 are:
                                 *   <total_receive_length>,<have_read_length>,<unread_length>
                                 * Only the third field is pending unread data. Treating the
                                 * already-read field as pending work causes stale probes such as
                                 * +QIRD: 182,182,0 to trigger empty fetch loops and retry noise. */
                                *unread_len = remaining_after_read;
                            } else if (fields >= 2) {
                                *unread_len = readable_now;
                            } else {
                                *unread_len = total_hint;
                            }
                        }
                    }
                } else if (strstr(line, "OK") != NULL) {
                    return saw_header != 0 ? 0 : -1;
                                      ...
                                      size_t min_len, size_t max_len, int allow_alpha)
{
    ...
    while (waited < 5000U) {
        ...
        if (ch == '\n') {
            line[li] = '\0';
            if (li > 0U) {
                (void)modem_maybe_handle_urc_line(line);
...
static void modem_prepare_at_rx(void)
{
    if (s_tcp_connected != 0) {
        /* Preserve queued unsolicited socket notifications while TCP is live.
         * Dropping +QIURC:"recv" here can leave unread downlink data stuck
         * in the modem until the whole TCP session is rebuilt. */
        stream_push_from_uart();
        modem_process_stream_lines();
        modem_http_discard_partial_line_residue();
        return;
    }
    modem_drain_hw_rx();
    s_rx_len = 0U;
...
int net_4g_modem_tcp_rx_fetch_direct(uint8_t *buf, size_t cap, size_t *out_nread)
{
    modem_qird_result_t qird;
    unsigned pending_before = 0U;
    unsigned request_len = 0U;
    uint8_t preserve_pending_hint = 1U;
    int fetch_rc;

    if (buf == NULL || out_nread == NULL || cap == 0U) {
        return -1;
    }
    *out_nread = 0U;
    if (s_tcp_connected == 0) {
    ...
    if (s_qird_pending_len == 0U) {
        unsigned total_len = 0U;
        unsigned unread_len = 0U;
        if (modem_qird_query_available(&total_len, &unread_len) != 0 || unread_len == 0U) {
            return 0;
        }
        s_qird_pending_len = unread_len;
        s_qird_pending_from_probe = 1U;
        if (modem_is_http_download_session() == 0) {
            char linebuf[144];
            (void)snprintf(linebuf, sizeof(linebuf),
                           "[4G] QIRD direct probe found unread=%u total=%u\r\n",
                           unread_len,
                           total_len);
            bsp_debug_log(linebuf);
        }
    ...
    pending_before = s_qird_pending_len;
    request_len = modem_qird_pick_direct_request_len(s_qird_pending_len, cap);
    if (request_len == 0U) {
        return 0;
    }

    fetch_rc = modem_qird_fetch_common(request_len, &qird, buf, cap);
    if (fetch_rc < 0) {
        s_qird_fail_streak++;
        if (s_qird_fail_streak >= 3U) {
            modem_reset_qird_pending_state();
            s_tcp_connected = 0;
            bsp_debug_log("[4G] QIRD direct fetch failed repeatedly, mark TCP disconnected\r\n");
        }
        return -1;
    }
    ...
    if (fetch_rc == 0) {
        if (modem_http_refresh_pending_after_empty_fetch(&s_qird_pending_len) != 0) {
            return net_4g_modem_tcp_rx_fetch_direct(buf, cap, out_nread);
        }
        return 0;
    }

    s_qird_pending_from_probe = 0U;
    if (qird.unread_len_known == 0U && request_len > 0U && qird.read_len >= (int)request_len) {
        preserve_pending_hint = 1U;
    }
    s_qird_pending_len = modem_qird_next_pending_len(pending_before, &qird, preserve_pending_hint);
    s_qird_pending_len = modem_http_refresh_pending_after_fetch(s_qird_pending_len, request_len, &qird);
    if (qird.data_len > 0U || qird.read_len > 0 || qird.unread_len > 0U) {
        modem_note_downlink_activity();
    }
    *out_nread = (size_t)((qird.read_len > 0) ? qird.read_len : 0);
    return 0;
}
...
static void modem_log_at_tx(const char *cmd)
{
    ...
    if (modem_is_http_download_session() != 0 && strncmp(cmd, "AT+QIRD=", 8) == 0) {
        return;
    }
...
static void modem_log_tcp_tx(const uint8_t *data, size_t len)
{
    ...
    if (modem_is_http_download_session() != 0) {
        shown = len > 120U ? 120U : len;
        printable = modem_ascii_window(data, shown, ascii, sizeof(ascii));
        if (printable > 0U) {
            (void)snprintf(line, sizeof(line),
                           "[TCP-TX] http len=%lu text=%s%s\r\n",
                           (unsigned long)len,
                           ascii,
                           len > shown ? "..." : "");
        } else {
            (void)snprintf(line, sizeof(line), "[TCP-TX] http len=%lu binary=1\r\n", (unsigned long)len);
        }
        bsp_debug_log(line);
        return;
    }
...
static size_t modem_ascii_window(const uint8_t *data, size_t len, char *out, size_t out_cap)
{
    ...
    for (i = 0U; i < len && pos + 1U < out_cap; i++) {
        uint8_t ch = data[i];
        if (ch >= 32U && ch <= 126U) {
            out[pos++] = (char)ch;
            printable++;
...
static void modem_log_tcp_rx(const uint8_t *data, size_t len)
{
    ...
    shown = len > 120U ? 120U : len;
    printable = modem_ascii_window(data, shown, ascii, sizeof(ascii));
    ascii_like = (shown > 0U && printable * 5U >= shown * 4U) ? 1 : 0;
    if (len >= 4U) {
        be_len = ((uint32_t)data[0] << 24) | ((uint32_t)data[1] << 16) |
                 ((uint32_t)data[2] << 8) | (uint32_t)data[3];
        if ((size_t)be_len + 4U <= len && data[4U] == '{') {
            json_len = len - 4U;
            if (json_len > 120U) {
                json_len = 120U;
            }
    ...
    if (modem_is_http_download_session() != 0) {
        if (ascii_like != 0 && len >= 5U && memcmp(data, "HTTP/", 5U) == 0) {
            modem_hex_window(data, len, hex, sizeof(hex));
            (void)snprintf(line, sizeof(line),
                           "[TCP-RX] http_header len=%lu text=%s%s hex=%s\r\n",
                           (unsigned long)len,
                           ascii,
                           len > shown ? "..." : "",
                           hex);
            bsp_debug_log(line);
        } else if (s_http_body_log_suppressed == 0U) {
            s_http_body_log_suppressed = 1U;
            modem_hex_window(data, len, hex, sizeof(hex));
            (void)snprintf(line, sizeof(line),
                           "[TCP-RX] HTTP body logging suppressed during OTA download len=%lu hex=%s\r\n",
                           (unsigned long)len,
                           hex);
            bsp_debug_log(line);
        }
    ...
    if (len >= 4U) {
        if (ascii_like != 0) {
            (void)snprintf(line, sizeof(line),
                           "[TCP-RX] chunk_len=%lu partial_prefix=%lu text=%s%s hex=%s\r\n",
                           (unsigned long)len,
                           (unsigned long)be_len,
                           ascii,
                           len > shown ? "..." : "",
            ...
            (void)snprintf(line, sizeof(line),
                           "[TCP-RX] chunk_len=%lu partial_prefix=%lu binary=1 hex=%s\r\n",
                           (unsigned long)len,
                           (unsigned long)be_len,
                           hex);
        }
    } else if (ascii_like != 0) {
        (void)snprintf(line, sizeof(line),
                       "[TCP-RX] short frame len=%lu text=%s%s hex=%s\r\n",
                       (unsigned long)len,
                       ascii,
                       len > shown ? "..." : "",
                       hex);
    } else {
        (void)snprintf(line, sizeof(line), "[TCP-RX] short frame len=%lu binary=1 hex=%s\r\n",
                                   ...
                                   uint8_t *direct_buf, size_t direct_cap)
{
    ...
    if (direct_buf != NULL && direct_cap == 0U) {
        return -1;
    }
    ...
    if (direct_buf != NULL && request_len > direct_cap) {
        request_len = (unsigned)direct_cap;
    }
    payload_timeout_ms = modem_qird_payload_byte_timeout_ms(direct_buf);
    {
        char logline[96];
    ...
    for (;;) {
        ...
        if (strncmp(line, "+QIRD:", 6) == 0) {
            ...
            int b = 0;
            int c = 0;
            header_fields = modem_parse_qird_header_values(line, &a, &b, &c);
            if (header_fields >= 3) {
                header_fields = 3;
    ...
    if (n_data > (int)request_len) {
        char linebuf[144];
        (void)snprintf(linebuf, sizeof(linebuf),
                       "[4G] QIRD header sanitized read_len=%d request_len=%u fields=%d\r\n",
                       n_data,
                       request_len,
                       header_fields);
        bsp_debug_log(linebuf);
        n_data = (int)request_len;
    }
    ...
    if (n_data <= 0) {
        if (result != NULL) {
            ...
            result->unread_len_known = (uint8_t)(header_fields == 3 ? 1U : 0U);
        }
...

    {
        ...
        while (remain > 0) {
            ...
            while (read_in_chunk < chunk) {
                int ch = modem_getch_ms(payload_timeout_ms);
                if (ch < 0) {
                    if (read_in_chunk > 0) {
                        if (direct_buf == NULL) {
                    ...
                    if (total_read > 0 || read_in_chunk > 0) {
                        char part[96];
                        if (direct_buf != NULL) {
                            (void)snprintf(part, sizeof(part),
                                           "[4G] QIRD partial direct payload read=%d/%d, abort fetch for clean retry\r\n",
                                           total_read + read_in_chunk,
                                           n_data);
                            bsp_debug_log(part);
                            modem_log_uart_rx_diag("partial_qird_direct");
                            return -1;
                        }
                        if (result != NULL) {
            ...
            if (direct_buf != NULL) {
                memcpy(direct_buf + total_read, tmp, (size_t)chunk);
            } else {
    ...
    modem_http_discard_partial_line_residue();
    /* Single-field +QIRD headers do not tell us the true unread tail.
     * For business TCP this can fabricate stale pending work like the classic
     * 330-byte phantom tail after a clean REGISTER_ACK read. For OTA HTTP it
     * can silently lose the true unread tail and feed a size-correct but
     * hash-wrong byte stream into proto_ota. After every successful single-
     * field fetch, issue one authoritative QIRD=0,0 probe and use its unread
     * count as the source of truth for follow-up pending work. */
    if (header_fields == 1) {
        unsigned probe_total = 0U;
        unsigned probe_unread = 0U;
        if (modem_qird_query_available(&probe_total, &probe_unread) == 0) {
            char linebuf[144];
            unread_len = probe_unread;
            if (result != NULL) {
                result->unread_len_known = 1U;
            }
            (void)snprintf(linebuf, sizeof(linebuf),
                           "[4G] QIRD boundary probe total=%u unread=%u after single-field fetch=%d/%u port=%u\r\n",
                           probe_total,
                           probe_unread,
                           n_data,
                           request_len,
                           (unsigned)s_tcp_remote_port);
            if (probe_unread > 0U || (unsigned)n_data != request_len) {
                bsp_debug_log(linebuf);
            }
        }
    }
...
{
    return modem_qird_fetch_common(request_len, result, NULL, 0U);
}
...
static void handle_urc_line(const char *line)
{
    ...
    if (strstr(line, "+QIURC: \"closed\"") != NULL) {
        ...
        tcp_fifo_reset();
        s_tcp_connected = 0;
        s_tcp_remote_port = 0U;
        s_http_body_log_suppressed = 0U;
    ...
    if (modem_is_http_download_session() == 0 || s_http_body_log_suppressed == 0U) {
    ...
    {
        unsigned request_len = modem_qird_pick_request_len(recv_len);
        if (modem_is_http_download_session() != 0 ||
            s_qird_pending_len > 0U ||
            request_len == 0U ||
            request_len < recv_len) {
            modem_accumulate_pending_recv(recv_len);
            return;
        }
    }
    {
        modem_qird_result_t qird;
        unsigned pending_before = recv_len;
        uint8_t preserve_pending_hint = 0U;
        ...
        s_qird_pending_from_probe = 0U;
        s_qird_pending_retry_after_ms = 0U;
        if (qird.unread_len_known == 0U && recv_len > 0U && qird.read_len >= (int)recv_len) {
            preserve_pending_hint = 1U;
        }
        s_qird_pending_len = modem_qird_next_pending_len(pending_before, &qird, preserve_pending_hint);
        s_qird_pending_len = modem_http_refresh_pending_after_fetch(s_qird_pending_len, recv_len, &qird);
        if (qird.data_len > 0U || qird.read_len > 0 || qird.unread_len > 0U) {
            modem_note_downlink_activity();
        }
        if (s_qird_pending_len > 0U) {
...
static int modem_maybe_handle_urc_line(const char *line)
{
    ...
    if (strstr(line, "+QIURC:") == NULL) {
        return 0;
    }
    {
        char dbg[176];
        (void)snprintf(dbg, sizeof(dbg), "[4G] URC during AT wait: %.148s\r\n", line);
        bsp_debug_log(dbg);
    }
...
void net_4g_modem_init(void)
{
    ...
    s_last_poll_monotonic_ms = 0U;
    s_last_downlink_activity_ms = 0U;
    s_last_time_sync_attempt_ms = 0U;
    s_last_identity_attempt_ms = 0U;
    s_last_signal_attempt_ms = 0U;
    s_last_qird_probe_ms = 0U;
    s_tcp_seen_downlink = 0U;
    s_tcp_remote_port = 0U;
    s_http_body_log_suppressed = 0U;
    modem_reset_qird_pending_state();
    modem_recovery_state_reset();
...
uint32_t net_4g_modem_downlink_idle_age_ms(uint32_t monotonic_ms)
{
    if (s_tcp_connected == 0 || s_tcp_seen_downlink == 0U || monotonic_ms < s_last_downlink_activity_ms) {
        return 0U;
    }
    return monotonic_ms - s_last_downlink_activity_ms;
}

int net_4g_modem_tcp_has_seen_downlink(void)
{
    return s_tcp_seen_downlink != 0U ? 1 : 0;
}
...
int net_4g_modem_tcp_connect(const char *host, uint16_t port)
{
    ...
    tcp_fifo_reset();
    s_tcp_connected = 1;
    s_tcp_remote_port = port;
    s_last_downlink_activity_ms = 0U;
    s_last_qird_probe_ms = 0U;
    s_tcp_seen_downlink = 0U;
    s_http_body_log_suppressed = 0U;
    bsp_system_delay_ms(MODEM_QIOPEN_READY_DELAY_MS);
...
int net_4g_modem_tcp_send(const uint8_t *data, size_t len)
{
...

    {
        ...
        for (attempt = 0U; attempt < 2U; attempt++) {
            modem_prepare_at_rx();
            if (attempt > 0U) {
                bsp_system_delay_ms(MODEM_QISEND_RETRY_DELAY_MS);
            }
            ...
            if (modem_wait_substr(">", MODEM_QISEND_PROMPT_TIMEOUT_MS) == 0) {
                break;
            }
            stream_push_from_uart();
            modem_process_stream_lines();
            ...
            if (attempt == 0U) {
                bsp_debug_log("[4G] TCP: QISEND prompt timeout, retry once\r\n");
            }
        }
        if (attempt >= 2U || s_tcp_connected == 0) {
            modem_reset_qird_pending_state();
            s_tcp_connected = 0;
...
{
    s_last_poll_monotonic_ms = monotonic_ms;
    stream_push_from_uart();
    modem_process_stream_lines();
    if (s_online != 0 && s_tcp_connected != 0 && s_qird_pending_len == 0U) {
        uint32_t probe_interval_ms =
            (modem_is_http_download_session() != 0) ? MODEM_QIRD_HTTP_PROBE_INTERVAL_MS
                                                    : MODEM_QIRD_PROBE_INTERVAL_MS;
        if (s_last_qird_probe_ms == 0U ||
            (uint32_t)(monotonic_ms - s_last_qird_probe_ms) >= probe_interval_ms) {
            unsigned total_len = 0U;
            unsigned unread_len = 0U;
            s_last_qird_probe_ms = monotonic_ms;
            if (modem_qird_query_available(&total_len, &unread_len) == 0) {
                if (unread_len > 0U) {
                    char linebuf[144];
                    s_qird_pending_len = unread_len;
                    s_qird_pending_from_probe = 1U;
                    (void)snprintf(linebuf, sizeof(linebuf),
                                   "[4G] QIRD active probe found unread=%u total=%u, fetch immediately\r\n",
                                   unread_len,
                                   total_len);
                    bsp_debug_log(linebuf);
                }
            } else {
                bsp_debug_log("[4G] QIRD active probe failed, keep current TCP session\r\n");
            }
        }
    }
    if (s_online != 0 && s_tcp_connected != 0 && s_qird_pending_len > 0U) {
        ...
        unsigned request_len = modem_qird_pick_request_len(s_qird_pending_len);
        modem_qird_result_t qird;
        if (request_len == 0U) {
            goto pending_qird_done;
        }
        ...
        {
            unsigned pending_before = s_qird_pending_len;
            uint8_t preserve_pending_hint = (uint8_t)((modem_is_http_download_session() != 0 ||
                                                       s_qird_pending_from_probe != 0U) ? 1U : 0U);
            ...
            if (fetch_rc < 0) {
                ...
                s_qird_fail_streak = 0U;
                if (s_qird_pending_from_probe != 0U) {
                    char linebuf[144];
                    (void)snprintf(linebuf, sizeof(linebuf),
                                   "[4G] QIRD active probe empty fetch remain=%u, clear stale probe hint\r\n",
                                   s_qird_pending_len);
                    bsp_debug_log(linebuf);
                    modem_reset_qird_pending_state();
                    goto pending_qird_done;
                ...
                if (s_qird_pending_no_data_streak >= MODEM_QIRD_PENDING_NO_DATA_LIMIT) {
                    char linebuf[160];
                    (void)snprintf(linebuf, sizeof(linebuf),
                                   "[4G] QIRD pending no-data streak=%u remain=%u, mark TCP disconnected for clean recovery\r\n",
                                   (unsigned)s_qird_pending_no_data_streak,
                                   s_qird_pending_len);
                    bsp_debug_log(linebuf);
                    modem_reset_qird_pending_state();
                ...
                s_qird_pending_len = modem_qird_next_pending_len(
                    pending_before,
                    &qird,
                    preserve_pending_hint);
                s_qird_pending_len = modem_http_refresh_pending_after_fetch(
                    s_qird_pending_len,
                    request_len,
                    &qird);
                if (s_qird_pending_len == 0U) {
