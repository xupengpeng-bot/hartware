...
#define NET_REGISTER_ACK_TIMEOUT_MS 15000U
...
static void net_log_endpoint_state(const char *event, const net_platform_endpoint_t *ep);
...
static void net_log_endpoint_state(const char *event, const net_platform_endpoint_t *ep)
{
    ...
    (void)snprintf(line, sizeof(line),
                   "[PROTO] %s host=%s port=%u\r\n",
                   event,
                   ep->tcp_host[0] != '\0' ? ep->tcp_host : "(empty)",
                   (unsigned int)ep->tcp_port);
    bsp_debug_log(line);
}
...
static void net_try_register(uint32_t monotonic_ms)
{
    ...
    if (rc >= 0) {
        bsp_debug_log("[PROTO] REGISTER send success, awaiting REGISTER_ACK\r\n");
...
void net_connectivity_poll(uint32_t monotonic_ms)
{
    ...
    if (s_sock.connected == 0 && ep->tcp_host[0] != '\0' && ep->tcp_port != 0U &&
        ...
        if (net_socket_client_connect(&s_sock, ep->tcp_host, ep->tcp_port) == 0) {
            net_log_endpoint_state("TCP connect success", ep);
            s_tcp_connected_since_ms = monotonic_ms;
            common_status_set_tcp_connected(true);
            s_register_pending = 1U;
            s_register_inflight = 0U;
        } else {
            net_log_endpoint_state("TCP connect failed", ep);
        }
    }
    ...
    for (uint32_t handled = 0U; handled < NET_MAX_INBOUND_FRAMES_PER_POLL; handled++) {
        ...
        if (feed_rc == NET_SOCKET_CLIENT_POLL_WRONG_UPSTREAM) {
            net_log_endpoint_state("wrong upstream on", ep);
    ...
    if (s_sock.connected != 0 && s_register_inflight != 0U && s_last_register_attempt_ms != 0U &&
        (uint32_t)(monotonic_ms - s_last_register_attempt_ms) >= NET_REGISTER_ACK_TIMEOUT_MS) {
        char line[192];

        (void)snprintf(line, sizeof(line),
                       "[PROTO] REGISTER_ACK timeout age=%lu ms, force reconnect host=%s port=%u\r\n",
                       (unsigned long)(monotonic_ms - s_last_register_attempt_ms),
                       ep->tcp_host[0] != '\0' ? ep->tcp_host : "(empty)",
                       (unsigned int)ep->tcp_port);
        bsp_debug_log(line);
        net_handle_disconnect("register_ack_timeout", monotonic_ms, 0U);
    }
}
