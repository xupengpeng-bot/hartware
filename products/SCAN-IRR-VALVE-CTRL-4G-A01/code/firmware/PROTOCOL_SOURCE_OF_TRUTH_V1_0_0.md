...
- stable queries: `qcs/qwf/qem/qgs/qgc`
- stable actions: `spu/tpu/pas/res/ppu/upg/rbt`
- pump supply switching is expressed by `bkr`, not `pdc`
- `upg` is manifest-driven OTA, and `rbt` is the released short code for remote MCU reboot
- `rbt` must ACK first and only reset after the ACK frame has been sent successfully
- `rbt` must reject while OTA workflow is active or an irrigation session is starting/running/stopping
