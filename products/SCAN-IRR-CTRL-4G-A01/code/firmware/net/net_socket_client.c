...
static int net_socket_contains_bytes(const uint8_t *data, size_t len, const char *literal)
{
    size_t lit_len;
    size_t i;
    ...
    lit_len = strlen(literal);
    if (lit_len == 0U || len < lit_len) {
        return 0;
    }
    for (i = 0U; i + lit_len <= len; i++) {
        if (memcmp(data + i, literal, lit_len) == 0) {
            return 1;
        }
    }
    return 0;
...
static int net_socket_is_wrong_upstream_payload(const net_socket_client_t *c)
{
    ...
    if (net_socket_starts_with_bytes(p, len, "HTTP/") != 0 ||
        ...
        net_socket_contains_bytes(p, sniff_len, "HTTP/1.") != 0 ||
        net_socket_contains_bytes(p, sniff_len, "Content-Type: text/") != 0 ||
        net_socket_contains_bytes(p, sniff_len, "<html") != 0 ||
        net_socket_contains_bytes(p, sniff_len, "<!doctype") != 0 ||
        net_socket_contains_bytes(p, sniff_len, "<meta ") != 0 ||
        net_socket_contains_bytes(p, sniff_len, "href=\"https://") != 0) {
        return 1;
    }
    return 0;
}
