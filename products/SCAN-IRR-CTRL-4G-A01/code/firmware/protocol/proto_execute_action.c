...
static int require_relay_target(const char *target)
{
    ...
    return strcmp(target, "relay_1") == 0 ? 0 : -1;
}
...
int proto_execute_action_handle(const char *json, size_t json_len, char *reply, size_t reply_cap)
{
    ...
    if ((strcmp(scope, "cm") == 0 || strcmp(scope, "common") == 0) && strcmp(action, "ppu") == 0) {
        ...
        return finalize_action_reply(reply, reply_cap, corr, scope, action, "controller",
                                     build_action_nack(reply, reply_cap, corr, session_ref,
                                                       scope, action, "controller", "MODULE_NOT_ENABLED", "voice disabled"));
    }
    ...
    if (strcmp(scope, "md") == 0 &&
        (strcmp(action, "ovl") == 0 || strcmp(action, "open_valve") == 0)) {
    ...
    if (strcmp(scope, "md") == 0 &&
        (strcmp(action, "cvl") == 0 || strcmp(action, "close_valve") == 0)) {
        if (require_valve_target(target) != 0) {
            if (params_json != NULL) {
                cJSON_free(params_json);
            }
    ...
    if (strcmp(scope, "md") == 0 &&
        (strcmp(action, "orl") == 0 || strcmp(action, "open_relay") == 0)) {
        if (require_relay_target(target) != 0) {
            ...
            cJSON_Delete(root);
            return finalize_action_reply(reply, reply_cap, corr, scope, action, target,
                                         build_action_nack(reply, reply_cap, corr, session_ref, scope, action, target, "PARAM_INVALID", "tr must be relay_1"));
        }
    ...
    if (strcmp(scope, "md") == 0 &&
        (strcmp(action, "crl") == 0 || strcmp(action, "close_relay") == 0)) {
        if (require_relay_target(target) != 0) {
            if (params_json != NULL) {
                cJSON_free(params_json);
            }
