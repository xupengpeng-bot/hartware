...
static int parse_feature_modules_array(const cJSON *arr, feature_modules_t *fm)
{
    ...
    cJSON_ArrayForEach(item, arr) {
        ...
        } else if (strcmp(item->valuestring, "dvl") == 0) {
            fm->dual_valve_control = 1U;
        } else if (strcmp(item->valuestring, "rly") == 0) {
            fm->relay_output_control = 1U;
...
static void parse_control_config(const cJSON *obj, control_config_t *cc)
{
    ...
    if (json_read_string(obj, "valve_control_mode", text, sizeof(text)) == 0) {
        if (strcmp(text, "relay_output") == 0) {
            cc->valve_control_mode = VALVE_CONTROL_RELAY_OUTPUT;
        } else if (strcmp(text, "pulse_output") == 0 ||
                   strcmp(text, "pwm_hold_output") == 0) {
            cc->valve_control_mode = VALVE_CONTROL_PULSE_OUTPUT;
        } else {
            cc->valve_control_mode = VALVE_CONTROL_DIRECT_OUTPUT;
        }
    }
    ...
    if (json_read_u32(obj, "valve_open_pulse_ms", &value_u32) == 0) {
        cc->valve_open_pulse_ms = (uint16_t)(value_u32 > 0xFFFFU ? 0xFFFFU : value_u32);
    }
    if (json_read_u32(obj, "valve_close_pulse_ms", &value_u32) == 0) {
        cc->valve_close_pulse_ms = (uint16_t)(value_u32 > 0xFFFFU ? 0xFFFFU : value_u32);
    }
...
static void normalize_pump_binding_to_meter(device_config_t *cfg)
{
    uint16_t i;
    channel_binding_t *binding = NULL;

    if (cfg == NULL) {
        return;
    }

    for (i = 0U; i < cfg->channel_binding_count && i < MODEL_MAX_CHANNEL_BINDINGS; i++) {
        if (strcmp(cfg->channel_bindings[i].channel_code, "pump_1") == 0) {
            binding = &cfg->channel_bindings[i];
            break;
        }
    }

    if (binding == NULL && cfg->channel_binding_count < MODEL_MAX_CHANNEL_BINDINGS) {
        binding = &cfg->channel_bindings[cfg->channel_binding_count++];
    ...
    (void)snprintf(binding->channel_code, sizeof(binding->channel_code), "%s", "pump_1");
    (void)snprintf(binding->module_code, sizeof(binding->module_code), "%s", "breaker_control");
    (void)snprintf(binding->channel_role, sizeof(binding->channel_role), "%s", "pump_run");
    (void)snprintf(binding->io_kind, sizeof(binding->io_kind), "%s", "rs485");
    (void)snprintf(binding->resource_ref, sizeof(binding->resource_ref), "%s", "rs485_meter_1");
    binding->enabled = 1U;
}

static void normalize_product_config(device_config_t *cfg)
{
    if (cfg == NULL) {
        return;
    }

    cfg->feature_modules.payment_qr_control = 0U;
    ...
    normalize_pump_binding_to_meter(cfg);
}
