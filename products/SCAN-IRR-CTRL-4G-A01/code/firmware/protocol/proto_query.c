                                            ...
                                            const runtime_state_t *rs, const common_status_t *cs)
{
    ...
    uint8_t meter_enabled = (uint8_t)(meter_protocol_value() != NULL);
    ...
    if (append_query_string_field(&jb, &first, "sc", scope) != 0 ||
        ...
        ((meter_enabled != 0U) && append_query_u32_field(&jb, &first, "me", rs->meter_epoch) != 0) ||
        ...
        ((meter_enabled != 0U) && append_query_fixed_field(&jb, &first, "ek", rs->energy_kwh, 2U) != 0)) {
        log_query_result_failure(qcode, "payload", -1, reply_cap);
        return -102;
    }
...
int proto_query_handle(const char *json, size_t json_len, char *reply, size_t reply_cap)
{
    ...
    if (scope_is_common(scope) && query_code_matches(qcode, "qem", NULL)) {
        if (meter_protocol_value() == NULL) {
            cJSON_Delete(root);
            return build_query_nack(reply, reply_cap, corr, session_ref, scope, qcode, "MODULE_NOT_ENABLED", "electric meter disabled");
        }
