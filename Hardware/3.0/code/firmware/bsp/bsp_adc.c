 ...
 * - PB12 drives BAT_TEST, enabling Q4.
...
{
    if (enable != 0) {
        GPIOB_BSRR = (1U << BOARD_HW_PIN_BAT_TEST_ENABLE_PORT_B);
    } else {
        GPIOB_BSRR = (1U << (BOARD_HW_PIN_BAT_TEST_ENABLE_PORT_B + 16U));
    }
}
...
static void stm32_adc_hw_init(void)
{
    ...
    /* PB12 push-pull output for BAT_TEST enable: MODE=11, CNF=00. */
    {
